...
// Optional dev-only seed for a developer account.
// Both credentials must be supplied via environment variables, and the server
// must NOT be running in production. No hardcoded fallbacks — if either env
// var is missing or NODE_ENV is "production", this is a no-op.
export async function ensureDevAccount(): Promise<void> {
  ...
  logger.info({ username }, "Seeded developer account from env");
}
